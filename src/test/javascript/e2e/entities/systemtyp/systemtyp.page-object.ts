...
export default class SystemtypComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('systemtyp-heading'));
...
export class SystemtypDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('gdb3App.systemtyp.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-systemtyp'));
}
