...
export default class HerstellerComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('hersteller-heading'));
...
export class HerstellerDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('gdb3App.hersteller.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-hersteller'));
}
