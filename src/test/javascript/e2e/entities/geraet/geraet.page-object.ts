...
export default class GeraetComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('geraet-heading'));
...
export class GeraetDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('gdb3App.geraet.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-geraet'));
}
