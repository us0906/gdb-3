...
/**
 * Performance test for the Systeminstanz entity.
 */
class SysteminstanzGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Systeminstanz entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all systeminstanzs")
            .get("/api/systeminstanzs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new systeminstanz")
            .post("/api/systeminstanzs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "bezeichnung":"SAMPLE_TEXT"
                , "geraetNummer":"SAMPLE_TEXT"
                , "geraetBaujahr":"SAMPLE_TEXT"
                , "gueltigBis":"2020-01-01T00:00:00.000Z"
                , "gwe":null
                , "bemerkung":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_systeminstanz_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created systeminstanz")
                .get("${new_systeminstanz_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created systeminstanz")
            .delete("${new_systeminstanz_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
