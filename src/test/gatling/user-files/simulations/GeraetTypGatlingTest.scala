...
/**
 * Performance test for the GeraetTyp entity.
 */
class GeraetTypGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the GeraetTyp entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all geraetTyps")
            .get("/api/geraet-typs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new geraetTyp")
            .post("/api/geraet-typs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_geraetTyp_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created geraetTyp")
                .get("${new_geraetTyp_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created geraetTyp")
            .delete("${new_geraetTyp_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
