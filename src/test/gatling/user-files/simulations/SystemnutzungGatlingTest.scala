...
/**
 * Performance test for the Systemnutzung entity.
 */
class SystemnutzungGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Systemnutzung entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all systemnutzungs")
            .get("/api/systemnutzungs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new systemnutzung")
            .post("/api/systemnutzungs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_systemnutzung_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created systemnutzung")
                .get("${new_systemnutzung_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created systemnutzung")
            .delete("${new_systemnutzung_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
