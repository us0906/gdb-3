...
/**
 * Performance test for the Zubehoer entity.
 */
class ZubehoerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Zubehoer entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all zubehoers")
            .get("/api/zubehoers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new zubehoer")
            .post("/api/zubehoers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_zubehoer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created zubehoer")
                .get("${new_zubehoer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created zubehoer")
            .delete("${new_zubehoer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
