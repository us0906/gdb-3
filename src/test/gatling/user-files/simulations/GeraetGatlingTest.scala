...
/**
 * Performance test for the Geraet entity.
 */
class GeraetGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Geraet entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all geraets")
            .get("/api/geraets")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new geraet")
            .post("/api/geraets")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_geraet_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created geraet")
                .get("${new_geraet_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created geraet")
            .delete("${new_geraet_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
