...
/**
 * Performance test for the Hersteller entity.
 */
class HerstellerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Hersteller entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all herstellers")
            .get("/api/herstellers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new hersteller")
            .post("/api/herstellers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_hersteller_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created hersteller")
                .get("${new_hersteller_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created hersteller")
            .delete("${new_hersteller_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
