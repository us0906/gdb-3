...
/**
 * Performance test for the Systemtyp entity.
 */
class SystemtypGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Systemtyp entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all systemtyps")
            .get("/api/systemtyps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new systemtyp")
            .post("/api/systemtyps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_systemtyp_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created systemtyp")
                .get("${new_systemtyp_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created systemtyp")
            .delete("${new_systemtyp_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
