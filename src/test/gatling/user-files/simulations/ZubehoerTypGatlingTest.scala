...
/**
 * Performance test for the ZubehoerTyp entity.
 */
class ZubehoerTypGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ZubehoerTyp entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all zubehoerTyps")
            .get("/api/zubehoer-typs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new zubehoerTyp")
            .post("/api/zubehoer-typs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_zubehoerTyp_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created zubehoerTyp")
                .get("${new_zubehoerTyp_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created zubehoerTyp")
            .delete("${new_zubehoerTyp_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
