...
/**
 * Performance test for the Betreiber entity.
 */
class BetreiberGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Betreiber entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all betreibers")
            .get("/api/betreibers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new betreiber")
            .post("/api/betreibers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created betreiber")
                .get("${new_betreiber_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created betreiber")
            .delete("${new_betreiber_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
